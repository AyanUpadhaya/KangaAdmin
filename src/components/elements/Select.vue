<template>
    <div class="relative select-none" ref="customSelect">
      <!-- default select  -->
      <div
        class="w-full border border-neutral-300 px-4 py-3 rounded-lg outline-none text-sm md:text-base flex items-center justify-between cursor-pointer "
        @click="toggleMenu"
        :class="selectedValue ? '' : 'text-disable'"
      > 
        <span class="ellipsis text-black-900">{{
      ...
      >
        <div
          class="bg-white w-full border border-t-0 left-0 rounded-lg max-h-64 overflow-auto custom-select"
        >
          <div class="py-2 px-4  font-poppins text-black-900 capitalize text-base">
            {{ placeholderValue }}
          </div>
          <div
            v-for="(item, index) in data"
            :key="index"
            class="py-2 px-4 hover:bg-gray-100 cursor-pointer duration-200 text-black-600 font-poppins font-normal"
