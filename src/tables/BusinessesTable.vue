<template>
      ...
      >
        <table className="table w-full table-pin-rows table-pin-cols ">
          ...
          <tbody v-if="currentRows.length > 0">
            <tr
              v-for="(businesses, i) in currentRows"
              :key="businesses._id"
            ...
            >
              ...
              <td class="py-3">
                <img :src="businesses?.image" class="w-10 h-10 rounded-md" alt="" />
              </td>
              <td class="py-3 text-black-medium-emp text-base font-normal font-poppins">
                {{
                  businesses?.name.length > 20
                    ? businesses?.name.slice(0, 20) + "..."
                    : businesses?.name
                }}
              </td>
              <td class="py-3 text-black-medium-emp text-base font-normal font-poppins">
              ...
              <td class="py-3 text-black-medium-emp text-base font-normal font-poppins  text-left">
                *{{
                  parseInt(businesses?.rating)
                  
                }}
              </td>
              <td class="py-3 text-black-medium-emp text-base font-normal font-poppins uppercase">
                {{
                 businesses?.package
              ...
              <td class="py-3  text-black-medium-emp text-base font-normal font-poppins flex justify-center">
                <button @click="()=>handleNavigate(businesses)" class=" text-center">
  ...
  export default {
    ...
    methods: {
      ...
      handleDelete() {
        this.$store.dispatch("deletebusinesses", { id: this.businessesId });
      },
      handleNavigate(item) {
        this.$router.push(`businesses-edit/:${item._id}`);
      },
